...
using jam::app::ConfigurationImpl;
...
class AppConfigurationTest : public testing::Test {
 public:
  ...
  jam::telemetry::TelemetryEndpoint get_telemetry_endpoint(
      const char *endpoint_uri, uint8_t verbosity_level) {
    auto uri = jam::common::Uri::parse(endpoint_uri);
  ...
  void SetUp() override {
    ...
    app_config_ = std::make_shared<ConfigurationImpl>();
  }
  ...
  std::shared_ptr<ConfigurationImpl> app_config_;
};
...
TEST_F(AppConfigurationTest, RocksDBStorageBackend) {
  ...
  ASSERT_TRUE(app_config_->initializeFromArgs(std::size(args), args));
  ASSERT_EQ(Configuration::StorageBackend::RocksDB,
