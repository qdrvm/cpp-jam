...
private:
    ...
    Result<void> recursive_search(const fs::path& dir_path, std::deque<std::shared_ptr<Module>>& modules);
    Result<void> load_module(const std::string& module_path, std::deque<std::shared_ptr<Module>>& modules);
};
