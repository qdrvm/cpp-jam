...
namespace jam::modules {
    ...
    Result<void> ModuleLoader::recursive_search(const fs::path& dir_path, std::deque<std::shared_ptr<Module>>& modules) {
        ...
        for (const auto& entry : fs::directory_iterator(dir_path)) {
            ...
            } else if (fs::is_regular_file(entry) && entry_path.extension() == ".so") {
    ...
    Result<void> ModuleLoader::load_module(const std::string& module_path, std::deque<std::shared_ptr<Module>>& modules) {
        std::unique_ptr<void, decltype(&dlclose)> handle(dlopen(module_path.c_str(), RTLD_LAZY), dlclose);
        if (!handle) {
            return Error::OpenLibraryFailed;
        }

        typedef const char* (*LoaderIdFunc)();
        LoaderIdFunc loader_id_func = (LoaderIdFunc) dlsym(handle.get(), "loader_id");
