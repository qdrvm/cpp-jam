...
#include <jam/snp/connections/stream.hpp>
...
inline auto operator""_ed25519(const char *c, size_t s) {
  auto seed = qtils::unhex<jam::ed25519::Seed>({c, s}).value();
  return jam::ed25519::from_seed(seed);
}
...
size_t indexOfKey(const Key &key) {
  auto it = std::ranges::find_if(keys, [&](const KeyPair &keypair) {
    return jam::ed25519::get_public(keypair) == key;
  });
  if (it == keys.end()) {
