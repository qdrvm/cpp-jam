...
template <typename Ret, typename Args>
struct fmt::formatter<morum::log::TraceFuncCall<Ret, Args>> {
  static constexpr auto parse(format_parse_context &ctx) {
    return ctx.begin();
  }
  static auto format(const morum::log::TraceFuncCall<Ret, Args> &v,
                     format_context &ctx) {
    auto out = ctx.out();
    ...
    if constexpr (not std::is_same_v<Ret, morum::log::TraceReturnVoid>) {
...
#define SL_TRACE_VOID_FUNC_CALL(logger, ...) \
  _SL_TRACE_FUNC_CALL(logger, ::morum::log::TraceReturnVoid{}, ##__VA_ARGS__)
