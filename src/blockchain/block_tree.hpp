...
namespace jam::blockchain {
   ...
   *    - work with blocks, which participate in the current round of block
  ...
  class BlockTree : public BlockHeaderRepository {
   public:
     ...
     * @return body, if the block exists in our storage, error in case it
     does
    ...
    virtual outcome::result<void> markAsRevertedBlocks(
        const std::vector<BlockHash> &block_hashes) = 0;
     ...
     */
    virtual outcome::result<std::vector<BlockHash>> getBestChainFromBlock(
        const BlockHash &block, uint64_t maximum) const = 0;
     ...
     * @return chain or blocks or error
     */
    virtual outcome::result<std::vector<BlockHash>> getDescendingChainToBlock(
        const BlockHash &block, uint64_t maximum) const = 0;
    ...
    //  */
    // virtual outcome::result<std::vector<BlockHash>> getChainByBlocks(
     ...
     * contain
     * @param max_number is the max block number that the resulting block
     (and
     ...
     */
    virtual outcome::result<std::vector<BlockHash>> getChildren(
