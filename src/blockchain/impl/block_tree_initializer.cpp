...
namespace jam::blockchain {
  ...
  namespace {
    ...
    outcome::result<std::set<BlockInfo>> loadLeaves(
      ...
      if (block_tree_leaves.empty()) {
        ...
        TimeSlot slot = 0;
        auto lower = std::numeric_limits<TimeSlot>::min();
        auto upper = std::numeric_limits<TimeSlot>::max();
  ...
  BlockTreeInitializer::BlockTreeInitializer(
    ...
    auto last_finalized_block_info_res = storage->getLastFinalized();
    if (last_finalized_block_info_res.has_error()) {
      SL_CRITICAL(logger,
                  "Failed to get last finalized block info: {}",
                  last_finalized_block_info_res.error());
      qtils::raise(last_finalized_block_info_res.error());
    }
    auto &last_finalized_block_info = last_finalized_block_info_res.value();
...

    {
      ...
      for (auto &leaf : block_tree_leaves) {
        ...
        for (auto block = leaf;;) {
          ...
          if (block.slot == last_finalized_block_info.slot) {
            dead.insert(subchain.begin(), subchain.end());
            ...
            for (;;) {
              ...
              if (fork_header.parent == main_header.parent) {
                break;
              }
          ...
          if (header.slot < last_finalized_block_info.slot) {
            SL_WARN(logger,
    ...
    SL_DEBUG(logger, "Last finalized block {}", last_finalized_block_info);

    last_finalized_ = last_finalized_block_info;
