...
namespace jam::storage {
  ...
  void configureColumnFamilies(
    ...
    for (auto &space_name : std::forward<ColumnFamilyNames>(cf_names)) {
      ...
      column_family_descriptors.emplace_back(
          space_name,
          configureColumn(space_name != spaceName(Space::TrieNode)
  ...
  RocksDb::RocksDb(log::Logger logger) : logger_(std::move(logger)) {
  ...
  outcome::result<std::shared_ptr<RocksDb>> RocksDb::create(
    ...
    OUTCOME_TRY(createDirectory(absolute_path, logger));
    ...
    const uint32_t other_spaces_cache_size =
        (memory_budget - trie_space_cache_size) / (SpacesCount - 1);
    ...
    if (!res.ok() && !res.IsPathNotFound()) {
      SL_ERROR(logger,
               ...
               res.ToString());
      return status_as_error(res, logger);
    }

    std::unordered_set<std::string> all_families;

    auto required_families =
        std::views::iota(0, (int)SpacesCount) | std::views::transform([](int i) {
    ...
    auto rocks_db = std::shared_ptr<RocksDb>(new RocksDb(logger));
